...
qpl_job* QplJobHWPool::AcquireDeflateJob(uint32_t& job_id) {
  ...
  uint32_t retry = 0;
  auto index = folly::Random::rand32(MAX_JOB_NUMBER/2, MAX_JOB_NUMBER - 1);
  while (!tryLockJob(index)) {
    index = folly::Random::rand32(MAX_JOB_NUMBER/2, MAX_JOB_NUMBER - 1);
    retry++;
  ...
  // std::cout << "AcquireDeflateJob job_id: " << job_id << std::endl;
