...
void IAAPageReader::preDecompressPage(bool& need_pre_decompress) {
  for (;;) {
...
PageHeader IAAPageReader::readPageHeader() {
...
const char* IAAPageReader::readBytes(int32_t size, BufferPtr& copy) {
  if (bufferEnd_ == bufferStart_) {
...
const bool FOLLY_NONNULL IAAPageReader::uncompressQplData(
    const char* pageData,
  ...
  if (!isWinSizeFit) {
    ...
    int window_size =
        getGzipWindowSize((const uint8_t*)pageData, uncompressedSize);
...
uint32_t IAAPageReader::DecompressAsync(
  ...
  if (job == nullptr) {
    ...
    return qpl_job_pool.MAX_JOB_NUMBER; // Invalid job id to illustrate the
...
const char* FOLLY_NONNULL IAAPageReader::uncompressData(
    const char* pageData,
    uint32_t compressedSize,
    uint32_t uncompressedSize) {
  switch (codec_) {
...
*/
int IAAPageReader::getGzipWindowSize(
...
void IAAPageReader::setPageRowInfo(bool forRepDef) {
...
void IAAPageReader::readPageDefLevels() {
...
void IAAPageReader::updateRowInfoAfterPageSkipped() {
  rowOfPage_ += numRowsInPage_;
  if (hasChunkRepDefs_) {
...
void IAAPageReader::prepareDict(
    const thrift::PageHeader& pageHeader,
    bool job_success) {
  if (!pre_decompress_dict || !job_success) {
    LOG(WARNING)
        << "Decompress w/IAA error, try to uncompress dict with software.";
...
bool IAAPageReader::prepareData(
    const thrift::PageHeader& pageHeader,
    int64_t row,
    bool job_success) {
  if (!pre_decompress_data || !job_success) {
    LOG(WARNING) << "Need to uncompress data with sw, pre_decompress_data: "
                 << pre_decompress_data << ", job_success: " << job_success;
  ...
  if (encodedDataSize_ > pageHeader.uncompressed_page_size) {
    LOG(WARNING) << "Decompress w/IAA error, false encodedDataSize_: "
...
void IAAPageReader::prepareDataPageV2(
    const PageHeader& pageHeader,
    int64_t row) {
...
void IAAPageReader::prepareDictionary(const PageHeader& pageHeader) {
  dictionary_.numValues = pageHeader.dictionary_page_header.num_values;
  dictionaryEncoding_ = pageHeader.dictionary_page_header.encoding;
  dictionary_.sorted = pageHeader.dictionary_page_header.__isset.is_sorted &&
      pageHeader.dictionary_page_header.is_sorted;
...
void IAAPageReader::makeFilterCache(dwio::common::ScanState& state) {
  VELOX_CHECK(
...
void IAAPageReader::preloadRepDefs() {
...
void IAAPageReader::decodeRepDefs(int32_t numTopLevelRows) {
...
int32_t IAAPageReader::getLengthsAndNulls(
    LevelMode mode,
...
void IAAPageReader::makeDecoder() {
  auto parquetType = type_->parquetType_.value();
  switch (encoding_) {
    case Encoding::RLE_DICTIONARY:
    case Encoding::PLAIN_DICTIONARY:
      if (encodedDataSize_ > dataPageHeader_.uncompressed_page_size) {
        std::cout << "encodedDataSize_: " << encodedDataSize_
                  << ", pre_decompress_data: " << pre_decompress_data << true
                  << ", dataPageHeader_: "
                  << dataPageHeader_.uncompressed_page_size << std::endl;
      }
...
void IAAPageReader::skip(int64_t numRows) {
...
int32_t IAAPageReader::skipNulls(int32_t numValues) {
...
void IAAPageReader::skipNullsOnly(int64_t numRows) {
  if (!numRows && firstUnvisited_ != rowOfPage_ + numRowsInPage_) {
    // Return if no skip and position not at end of page or before first page.
    return;
  }
...
void IAAPageReader::readNullsOnly(int64_t numValues, BufferPtr& buffer) {
...
IAAPageReader::readNulls(int32_t numValues, BufferPtr& buffer) {
  if (maxDefine_ == 0) {
    buffer = nullptr;
    return nullptr;
  }
...
void IAAPageReader::startVisit(folly::Range<const vector_size_t*> rows) {
...
bool IAAPageReader::rowsForPage(
...
const VectorPtr& IAAPageReader::dictionaryValues(const TypePtr& type) {
...
bool IAAPageReader::waitQplJob(uint32_t job_id) {
  dwio::common::QplJobHWPool& qpl_job_pool =
      dwio::common::QplJobHWPool::GetInstance();
