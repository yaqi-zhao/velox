 ...
 public:
    DeflateRleBpDecoder(
        ...
        : pageData_(pageData),
          ...
          encodedSize_(encodedSize),
          type_(type),
          pool_(pool),
          decoded_values_(NULL),
          cur_visitor_pos(0) {
            bitWidth_ = pageData[0];
          }

  uint32_t rleDecode() {
    // Step1. uncompress + scan with mask data pate to get output vector
    using TValues = typename std::remove_reference<int32_t>::type;
  ...
  uint32_t rleDecompress() {
    ...
    VELOX_DCHECK(status == QPL_STS_OK, "Execturion of QPL Rle && Decompress Job failed, status {}, job_id {}", status, (int)job_id);
    if (status != QPL_STS_OK) {
    throw std::runtime_error(
        "submition of QPL Job failed, status:" + std::to_string(status) + " job_id: " + std::to_string(job_id));
    }
