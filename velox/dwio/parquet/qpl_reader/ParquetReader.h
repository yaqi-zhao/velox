...
class ReaderBase {
 public:
  ReaderBase(
      ...
      const facebook::velox::dwio::common::ReaderOptions& options);
  ...
  facebook::velox::dwio::common::BufferedInput& bufferedInput() const {
    return *input_;
  }
  ...
  const std::shared_ptr<const facebook::velox::dwio::common::TypeWithId>& schemaWithId() {
  ...
  int64_t rowGroupUncompressedSize(
      ...
      const facebook::velox::dwio::common::TypeWithId& type) const;
 ...
 private:
  ...
  const facebook::velox::dwio::common::ReaderOptions& options_;
  std::unique_ptr<facebook::velox::dwio::common::BufferedInput> input_;
  uint64_t fileLength_;
  std::unique_ptr<thrift::FileMetaData> fileMetaData_;
  RowTypePtr schema_;
  std::shared_ptr<const facebook::velox::dwio::common::TypeWithId> schemaWithId_;

  const bool binaryAsString = false;

  // Map from row group index to pre-created loading BufferedInput.
  std::unordered_map<uint32_t, std::unique_ptr<facebook::velox::dwio::common::BufferedInput>>
      inputs_;
};

/// Implements the RowReader interface for Parquet.
class ParquetRowReader : public facebook::velox::dwio::common::RowReader {
 public:
  ParquetRowReader(
      const std::shared_ptr<ReaderBase>& readerBase,
      const facebook::velox::dwio::common::RowReaderOptions& options);
  ~ParquetRowReader() override = default;

  uint64_t next(uint64_t size, velox::VectorPtr& result) override;

  void updateRuntimeStats(
      facebook::velox::dwio::common::RuntimeStatistics& stats) const override;

  void resetFilterCaches() override;

  std::optional<size_t> estimatedRowSize() const override;

  const facebook::velox::dwio::common::RowReaderOptions& getOptions() {
    return options_;
  }
 ...
 private:
  ...
  const facebook::velox::dwio::common::RowReaderOptions& options_;
  ...
  std::unique_ptr<facebook::velox::dwio::common::SelectiveColumnReader> columnReader_;

  RowTypePtr requestedType_;
};

/// Implements the reader interface for Parquet.
class ParquetReader : public facebook::velox::dwio::common::Reader {
 public:
  ParquetReader(
      std::unique_ptr<facebook::velox::dwio::common::BufferedInput>,
      const facebook::velox::dwio::common::ReaderOptions& options);
  ...
  std::unique_ptr<facebook::velox::dwio::common::ColumnStatistics> columnStatistics(
  ...
  const std::shared_ptr<const facebook::velox::dwio::common::TypeWithId>& typeWithId()
      const override {
    return readerBase_->schemaWithId();
  }

  std::unique_ptr<facebook::velox::dwio::common::RowReader> createRowReader(
      const facebook::velox::dwio::common::RowReaderOptions& options = {}) const override;

 private:
  std::shared_ptr<ReaderBase> readerBase_;
};

class ParquetReaderFactory : public facebook::velox::dwio::common::ReaderFactory {
 public:
  ParquetReaderFactory() : ReaderFactory(facebook::velox::dwio::common::FileFormat::PARQUET) {}
