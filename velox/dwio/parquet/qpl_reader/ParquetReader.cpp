...
#include "velox/dwio/parquet/qpl_reader/StructColumnReader.h"
...
void ReaderBase::loadFileMetaData() {
  ...
  auto stream = input_->read(
      fileLength_ - readSize, readSize, facebook::velox::dwio::common::LogType::FOOTER);

  std::vector<char> copy(readSize);
  const char* bufferStart = nullptr;
  const char* bufferEnd = nullptr;
  facebook::velox::dwio::common::readBytes(
  ...
  if (footerLength > readSize - 8) {
    ...
    stream = input_->read(
        ...
        missingLength,
        facebook::velox::dwio::common::LogType::FOOTER);
...
std::shared_ptr<const ParquetTypeWithId> ReaderBase::getParquetColumnInfo(
  ...
  if (!schemaElement.__isset.type) { // inner node
    ...
    std::vector<std::shared_ptr<const facebook::velox::dwio::common::TypeWithId>> children;
...
void ReaderBase::scheduleRowGroups(
  ...
  if (!input) {
    ...
    newInput->load(facebook::velox::dwio::common::LogType::STRIPE);
...
namespace {
struct ParquetStatsContext : facebook::velox::dwio::common::StatsContext {};
} // namespace
...
void ParquetRowReader::updateRuntimeStats(
    facebook::velox::dwio::common::RuntimeStatistics& stats) const {
...
std::unique_ptr<facebook::velox::dwio::common::RowReader> ParquetReader::createRowReader(
    const facebook::velox::dwio::common::RowReaderOptions& options) const {
